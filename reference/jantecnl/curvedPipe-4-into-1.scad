...
use <libs/moreShapes.scad> // Author: Damian Axford, with elements by nophead, Public Domain
...
module pipeCurve(points, point, radii, od, id, segments, isLastSegment = false)
{
    ...
    // somewhere over here, a memory function needs to be built to NOT make a wall when it collides with the previous
    // walls, and vice versa!  Then, the inner area of crossing pipes will remain empty, instead of blocked by the
    // crossing pipe!  This can be best done by running each pipe section with numbered naming and use all others as
    // cutouts for any run... Check for how the Internl diameter is cutoff, this might be useful as example as to WHERE
    // put the cutoffs..  OR- an easier approach could be to just cutout ALL internal diameters for all pipes
    // everywhere....

    // end of the tubes write command
    translate(mid) orientate(dir2) translate([ 0, 0, inset ])
        tube(h = l2 - postInset - inset, or = od / 2, ir = id / 2,
             center = false); // THE ACTUAL 'tube' WRITE ASSIGNMENT for the end of the pipe
    ...
    //}
    // THE ACTUAL 'torusSlice' WRITE ASSIGNMENT where we must solve the problem that the pipes block each other upon
    // multiple pipes when bending in each other's way
...
// The following module does the actual WRITE function of the pipe's curved coutouts for each part and is repeated as
// many times as required to write all pipes:
module pipeCurveCutout(points, point, radii, od, id, segments, isLastSegment = false)
{
    ...
    // start of the tubes write command
    translate(start) orientate(dir1) translate([ 0, 0, preInset ])
        tube(h = l1 - preInset - inset, or = id / 2, ir = 0, center = false);
    ...
    // end of the tubes write command
    translate(mid) orientate(dir2) translate([ 0, 0, inset ])
        tube(h = l2 - postInset - inset, or = id / 2, ir = 0,
             center = false); // THE ACTUAL 'tube' WRITE ASSIGNMENT for the end of the pipe
    ...
    translate(rStart) pipeOrientate(dir1, dir2) rotate([ 0, 0, 180 ]) // rotate to lie along x
        ...
        translate([ -r, 0, 0 ])
            torusSlice_only_inner_pipes(r1 = r, r2 = od / 2, r3 = id / 2, start_angle = 0, end_angle = ang);
...
// The following module does the actual WRITE function for the pipes and is repeated as many times as required to write
// all pipes:
