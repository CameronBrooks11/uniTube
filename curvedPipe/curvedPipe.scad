...
module pipeOrientate(v1, v2)
{
	...
	rotate(a = theta != 0 ? theta : 0, v = [ 0, 0, 1 ]) 
	//rotate(a = 90, v = [ 0, 1, 0 ]);
	children(0);
}
...
module pipeCurve(points, point, radii, od, id, isLastSegment = false)
{
    ...
    translate(rStart) 
        ...
        rotate([0, 0, 180])   // Rotate to align along x-axis
        rotate([90, 0, 0])    // Flip up
        translate([-r, 0, 0]) 
        torusSlice(r1 = r, r2 = od / 2, r3 = id / 2, start_angle = 0, end_angle = ang);
}
