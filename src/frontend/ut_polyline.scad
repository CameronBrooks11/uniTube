...
function ut_polyline(pts, r = 0, closed = false, frame = "transport", normal = undef,
    ...
    assert(is_num(r) || len(r) == len(pts) || len(r) == len(pts) - 2,
        ...
        assert(closed == false, "ut_polyline(): closed paths land in a later phase")
            let(n = len(pts), dirs = [for (i = [0:n - 2]) pts[i + 1] - pts[i]], lens = [for (d = dirs) norm(d)],
                // vertex-indexed; ends are 0
                angs = [for (k = [0:n - 1])(k == 0 || k == n - 1) ? 0 : _ut_defl(dirs[k - 1], dirs[k], k, pts)],
...
function _ut_defl(d1, d2, k, pts) = let(a = ut_turn(d1, d2))
    assert(a < 180 - 1e-6,
           str("ut_polyline(): 180 degree reversal at vertex ", k, " ", pts[k],
               " -- a path cannot double back on itself (tan(90) is inf, and the segment would silently vanish)")) a;
