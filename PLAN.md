...
| **Biarc fitting** for bezier/spline | Highest-value deferred item | Approximates each span by tangent-continuous arcs to a tolerance, emitting real `["A"]` segments — restoring exact transport, exact arclength and the bend-radius check for freeform paths. CAM systems have done this since the 1980s. |
